...
function init_params {
    ...
    model=${model_name}
    if [ ${model} == "hbonet-0.25-224x224" ]; then
        width_mult=0.25
    elif [ ${model} == "hbonet-0.5-224x224" ]; then
        width_mult=0.5
    elif [ ${model} == "hbonet-1.0-224x224" ]; then
        width_mult=1.0
    fi
}
    ...
    if [ "$run_perf" == "1" ];then
        ...
        do
            ...
            printf "numactl --localalloc --physcpubind ${cpu_array[i]} \
                    timeout 3600  python imagenet.py \
                        --dummy \
                        -b ${bs} \
                        -a hbonet \
                        -e \
