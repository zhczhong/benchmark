    ...
    if [ "$run_perf" == "1" ];then
        ...
        do
            ...
            printf "numactl --localalloc --physcpubind ${cpu_array[i]} \
                    timeout 3600  python train.py --epochs 1 --warmup-factor 0 --lr 2.5e-3 --no-save --threshold=0.23 \
                                --data /home2/pytorch-broad-models/COCO2017 --evaluation 1 --ipex --eval-only \
                                --perf-prerun-warmup 10 --precision ${precision} \
