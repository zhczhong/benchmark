...
function generate_core {
    ...
    do
        ...
        printf " OMP_NUM_THREADS=${real_cores_per_instance} numactl --localalloc --physcpubind ${cpu_array[i]} \
            python train.py --epochs 1 --warmup-factor 0 --lr 2.5e-3 --no-save --threshold=0.23 \
            --data /lustre/dataset/COCO2017 --batch-size 1 --evaluation 1 --ipex --eval-only --perf-prerun-warmup 10 \
            > ${log_file} 2>&1 & " |tee -a ${excute_cmd_file}
            # --checkpoint /tmp/ssd-model/iter_0.pt
    done
