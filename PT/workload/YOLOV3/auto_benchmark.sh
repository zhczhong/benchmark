...
cp ${PATCH_DIR}/YoLov3.patch .
cp ${PATCH_DIR}/launch_benchmark_yolov3.sh .
cp ${PATCH_DIR}/launch_benchmark_yolov3-tiny.sh .
cp ${oob_home_path}/parsednn.py .
git apply YoLov3.patch
pip install -r requirements.txt

rm -rf weights 
ln -s ${pytorch_pretrain_dir}/YOLOV3/models/ weights

MYDIR=`pwd`
echo $MYDIR

bash ./launch_benchmark_yolov3.sh --model_name="Yolo3" \
                        --precision=${precision} \
                        --run_perf=${run_perf} \
                        --collect_dnnl_verbose=${collect_dnnl_verbose} \
                        --workspace=${workspace} \

bash ./launch_benchmark_yolov3-tiny.sh --model_name="Yolo3-tiny" \
                        --precision=${precision} \
                        --run_perf=${run_perf} \
                        --collect_dnnl_verbose=${collect_dnnl_verbose} \
                        --workspace=${workspace} \
