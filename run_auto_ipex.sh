...
for model in ${MODEL_NAME_LIST[@]}
do
    numactl --cpunodebind=0 --membind=0 python main.py -e --performance --pretrained --dummy --no-cuda -j 1 -w 20 -i 200 -a $model -b 80 --precision "int8_ipex" --ipex --profile 2>&1 | tee ./logs/$model-IPEX-INT8.log
    ...
    echo $model IPEX INT8 $latency $throughput | tee -a ./logs/summary.log
done
