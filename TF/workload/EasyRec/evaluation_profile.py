  ...
  with monitored_session.MonitoredSession(
    ...
    if eval_ops is not None:
      while not session.should_stop():
        tic = time.time()
        session.run(eval_ops, feed_dict, options=run_options, run_metadata=run_metadata)
        toc = time.time()
        if i >= 50:
          total_time += (toc - tic)
          total_iters += 1
        if i == 200:
          ...
          if not os.path.exists(timeline_dir):
              try:
                  ...
                  pass
          timeline_file = timeline_dir + 'timeline-' + str(i) + '-' + str(os.getpid()) + '.json'
          with open(timeline_file, 'w') as trace_file:
