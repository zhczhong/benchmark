...
function main {
    ...
    if [ "${precision}" == "float32" ];then
        precision="fp32"
        input_graph=/home2/tensorflow-broad-product/oob_tf_models/intel-models/tf_dataset/pre-trained-models/SSDMobilenet/fp32/ssdmobilenet_fp32_pretrained_model_combinedNMS.pb
    elif [ "${precision}" == "int8" ];then
        input_graph=/home2/tensorflow-broad-product/oob_tf_models/intel-models/tf_dataset/pre-trained-models/SSDMobilenet/int8/ssdmob_int8_new.pb
    else
        input_graph=/home2/tensorflow-broad-product/oob_tf_models/intel-models/tf_dataset/pre-trained-models/SSDMobilenet/fp32/ssdmobilenet_fp32_pretrained_model_combinedNMS.pb
    fi
    #
    ...
    do
        ...
        printf "numactl -m $(echo ${cpu_array[i]} |awk -F ';' '{print $2}') \
            ...
            python ./benchmarks/launch_benchmark.py --benchmark-only --framework tensorflow \
                --model-name ssd-mobilenet --mode inference \
